...
public class MarginPerceptron{
  public static int dim = 123;
  public static int update_time = 0;
  public MarginPerceptron(int d, int u){
    this.dim = d;
    this.update_time = u;
  }
  ...
  public static List<Double> produceW(List<String> table, double r, double u, int dimension, List<Double> w){
      int cnt_0 = 0;
      for(String i : table){
  ...
  public static void run(int epo, boolean flag){
   ...
   for(Double u : uu)
    for(Double r : rr){
     ...
     for(int k = 1; k <= 50; k ++){
      ...
      if(flag){
       ...
       for(int epoch = 1; epoch <= epo; epoch ++){
          ...
          w = produceW(tmp, r, u, dim, w);
       }
      }
      else
        w = produceW(table, r, u, dim, w);
      for(int i = 0; i < w.size(); i ++)
        ww[i] += w.get(i);
     ...
     System.out.println();
     System.out.println("update time:" + update_time / 50);
     System.out.println("u:" + u + "r:" + r + "train accuracy:" + (1 - ans / 50));
     System.out.println("u:" + u + "r:" + r + " test accuracy:" + (1 - ans1 / 50) + "\n");
    }
